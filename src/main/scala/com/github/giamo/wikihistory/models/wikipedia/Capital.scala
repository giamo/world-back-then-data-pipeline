...
object Capital {
  ...
  def fromString(s: String): Capital = s match {
    case complexCapitalRegex(c, d) if Date.fromString(d).isRight => Capital(c, Date.fromString(d).toOption)
    case linkCapitalRegex(c, d) if Date.fromString(d).isRight => Capital(c, Date.fromString(d).toOption)
    case simpleCapitalRegex(c, d) if Date.fromString(d).isRight => Capital(c, Date.fromString(d).toOption)
    case _ => Capital(s)
  }
}
